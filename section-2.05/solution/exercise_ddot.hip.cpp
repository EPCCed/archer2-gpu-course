...
/* Suggested procedure
     1. Use a __shared__ temporary variable to store the contribution from each
     different thread in a block,
        and then compute the sum for the block.
...
__global__ void ddot_parallel(int n, double *x, double *y, double *result) {
  ...
  if (threadIdx.x == 0) {
    for (i = 0; i < THREADS_PER_BLOCK; i++) {
     ...
     atomicAdd(result, sum); //*result = sum;
  }
}

/* It is possible to use solely atomicAdd() to form the result (and not do
   anything using __shared__ within a block)? Investigate the performance
