...
SIMT (Single Instruction Multiple Thread) unit. In AMD CDNA (data-centre) GPUs,
wavefronts consist of 64 threads. In Nvidia GPUs a wavefront is called a
*warp* and consists of 32 threads.

Wavefronts are mapped to Compute Units (CU) for execution by the hardware
scheduler. In Nvidia documentation, a Compute Unit is called a Streaming
Multiprocessor (SM).
