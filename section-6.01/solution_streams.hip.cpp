...
int main(int argc, char *argv[]) {
  ...
  HIP_ASSERT(hipMemcpyAsync(d_x, h_x, mrow * sizeof(double), kind, stream));
  ...
  myKernel<<<blocks, threadsPerBlock, 0, stream>>>(mrow, ncol, alpha, d_x, d_y, d_a);
  ...
  HIP_ASSERT(hipFreeHost(h_y));
  HIP_ASSERT(hipFreeHost(h_x));
  delete h_a;
